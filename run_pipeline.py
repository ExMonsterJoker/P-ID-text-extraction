...
from configs import get_config  # Import the get_config helper
...
def run_slicing_step(input_dir: str):
    ...
    slicer = SahiSlicer()  # Instantiated without config
...
def run_text_detection_step():
    ...
    detector = text_detection()  # Instantiated without config
...
def run_grouping_and_filtering_step(grouping_args: argparse.Namespace):
    ...
    grouper = BoundingBoxGrouper()  # Instantiated without config
...
def run_coordinate_conversion_step():
    ...
    re_ocr_config = get_config('re_ocr') # Note: This might need adjustment if re_ocr is removed

    input_dir_json = re_ocr_config.get('output_dir', 'data/processed/metadata/final_annotations')
...
def run_visualization_step():
    ...
    pipeline_config = get_config('pipeline')
    # The annotation source is now the output of the 'recognize' step
...
def run_text_recognition_step_pipeline():
    ...
    run_text_recognition_step() # No config passed
